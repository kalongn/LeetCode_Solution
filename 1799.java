class Solution {
    public int maxScore(int[] nums) {
        ...
        // 'dp[i]' stores max score we can get after picking remaining numbers
        // represented by 'i'.
        int[] dp = new int[maxStates];

        // Iterate on all possible states one-by-one.
        for (int state = finalMask; state >= 0; state--) {
            // If we have picked all numbers, we know we can't get more score as no number
            // is remaining.
            if (state == finalMask) {
                dp[state] = 0;
                continue;
            ...
            // We have picked 'pairsFormed' pairs, we try all combinations of one more pair
            // now.
